(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@bilibili/js-bridge')) :
	typeof define === 'function' && define.amd ? define(['exports', '@bilibili/js-bridge'], factory) :
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.OpenBcut = {}, global.jsBridge));
})(this, (function (exports, jsBridge) { 'use strict';
	...
	*/
	...
	var isIOS = /\(i[^;]+;( U;)? CPU.+Mac OS X/i.test(ua); // 中间承载页面

	var middlePage = 'https://www.bilibili.com/blackboard/activity-GD74tr1a4K.html?id=1955295#/guide?tabid=750124&modelid=1955295'; // 唤醒必剪的默认scheme

	var DEFAULT_SCHEME = "bcut://"; // IOS AppStore地址

	var IOS_APPSTORE_URL = "https://apps.apple.com/cn/app/%E5%BF%85%E5%89%AA/id1299589486"; // 安卓下载地址

	var ANDROID_DOWNLOAD_URL = "https://dl.hdslb.com/mobile/latest/bilistudio/android_bbs-master.apk"; // 安卓端包名

	var PACKAGE_NAME = "com.bilibili.studio";
	var bcut_timeout = null;

	var isAppSupport = function isAppSupport(methodName) {
	  return new Promise(function (resolve, reject) {
	    jsBridge.isSupport(methodName).then(function (support) {
	      if (support) {
	        resolve(1);
	      } else {
	...
	var callNative = function callNative(methodName, data) {
	  return new Promise(function (resolve, reject) {
	    isAppSupport(methodName).then(function () {
	      jsBridge.biliBridge.callNative({
	        method: methodName,
	        data: data,
	...
	var isInstalled = function isInstalled(packageName, url) {
	  if (!jsBridge.inBiliApp) return Promise.reject();

	  if (isAndroid) {
	    return callNative('utils.isInstalled', {
	      packageName: packageName
	    }).then(function (data) {
	      return data && data.isInstalled;
	    });
	  } else {
	...
	var openApp = function openApp(scheme) {
	  if (!jsBridge.inBiliApp) {
	...
	var getDownloadLink = function getDownloadLink() {
	  var browser = getDeviceType();

	  if (jsBridge.inBiliApp && browser.android) {
	...
	var goAppStore = function goAppStore(iosStoreURL) {
	  if (!jsBridge.inBiliApp) {
	    return Promise.reject();
	  }
	...
	var outOpenBcut = function outOpenBcut(opt) {
	  ...
	  bcut_timeout = setTimeout(function () {
	    // 唤醒超时跳转至下载页
	    if (isAndroid) {
	...
	var openBcut = function openBcut(config) {
	  var scheme = DEFAULT_SCHEME;
	  var iosStoreURL = IOS_APPSTORE_URL;
	  var androidURL = ANDROID_DOWNLOAD_URL;
	  var packageName = PACKAGE_NAME;
	  var middlePageURl = middlePage;
	  ...
	  if (!jsBridge.inBiliApp) {
	    outOpenBcut({
	      ...
	      middlePageURl: middlePageURl
	    });
	    return Promise.resolve();
	  } else {
	    return isInstalled(packageName, scheme).then(function (installed) {
	      if (installed) {
